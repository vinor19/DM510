...
struct lfs_file{
	...
	int maxSize; //Current max size of folder/file
	int entries; //Number of entries in folder;
	...
	//Contents
	struct lfs_file **dir; //If folder, this will be what is in the folder, if not it will be Null
...
//Takes an array of strings, and builds it into a path
const char *pathRebuild(char**path, int length){
	...
	//Finds char length of the path that will be made
...
//Finds a file by using a given path
struct lfs_file *findFile(const char* path){
	...
	if(strcmp(path,"/") == 0){
		...
		for(int i = 0; i < depth;i++){ //Loops over path
			//printf("I: %d\n", i);
			for(int j = 0; j < foundfile->size && foundfile->dir != NULL; j++){ //Loops over current folder
				//printf("J: %d\n",j);
		...
		if (strcmp(splitpath[depth-1], foundfile->name) == 0 && depth == foundfile->depth){ //Makes sure it's the actual file and hasn't just found/stopped in an earlier folder
...
//Aux for making folder and directories
int lfs_putIntoParent(struct lfs_file* file){
	if(file->depth == 1){
		//printf("Creating in root directory\n");
		if(root->size >= root->maxSize){ //updates folder size if folder is full
		...
		if (foundfile == NULL)
			return -ENOENT;
		if(foundfile->size >= foundfile->maxSize){ //updates folder size if folder is full
...
int lfs_mkdir(const char *path, mode_t mode) {
	...
	return lfs_putIntoParent(file);
}

//Removes the folder with that path
int lfs_rmdir(const char *path){
	...
	if(folderDelete->entries > 0) //Checks if the folder is empty or not
		return -ENOTEMPTY;
	...
	for(int i = 0; i<folderDeleteParent->maxSize;i++){
		...
		if(folderDeleteParent->dir[i] != NULL){
			...
			if(strcmp(folderDeleteParent->dir[i]->name,folderDelete->name)==0){
				//printf("Found it\n");
				folderDeleteParent->dir[i] = NULL; //Sets the pointer to null
	...
	//Frees all allocated memory of the folder
	free(folderDelete->path);
	free(folderDelete->name);
	free(folderDelete->dir);
	free(folderDelete);
	//printf("Done freeing\n");
	//Last updates to the parent folder
